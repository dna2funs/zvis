...
const i_env = {
   debug: !!process.env.ZVIS_DEBUG,
   server: {
      host: process.env.ZVIS_HOST || '127.0.0.1',
      port: parseInt(process.env.ZVIS_PORT || '8080'),
      staticDir: (
         process.env.ZVIS_STATIC_DIR?
         i_path.resolve(process.env.ZVIS_STATIC_DIR):null
      ),
      httpsCADir: (
         process.env.ZVIS_HTTPS_CA_DIR?
         i_path.resolve(process.env.ZVIS_HTTPS_CA_DIR):null
      ),
   },
};
...
const utils = {
   ...
   readRequestJson: async (req) => {
      const bin = await utils.readRequestBinary(req);
      try {
...
const GRAPH_FILE = process.env.ZVIS_GRAPH_FILE;
const server = createServer({
   ...
   api: { v0: {
      load: (req, res, opt) => {
         i_fs.readFile(GRAPH_FILE, (err, buf) => {
            if (err) return utils.r500(res);
            utils.rJson(res, JSON.parse(buf));
         });
      },
      save: async (req, res, opt) => {
         const json = await utils.readRequestJson(req);
         if (!json) return utils.r500(res);
         i_fs.writeFile(GRAPH_FILE, JSON.stringify(json), (err) => {
            if (err) return utils.r500(res);
            utils.r200(res);
         });
      }
...
server.listen(i_env.server.port, i_env.server.host, () => {
   console.log(`zvis server is listening at ${i_env.server.host}:${i_env.server.port}`);
});
